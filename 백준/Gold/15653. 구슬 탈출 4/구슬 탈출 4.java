...
public class Main {
	...
	static Set<String> marbles = new HashSet<>();
	...
	public static int bfs() {
		...
		marbles.add(convert(red[0], red[1]) + convert(blue[0], blue[1]));
		...
		while (true) {
			...
			for (int i = 0; i < 4; i++) {
				...
				if (result == 0 && !isSame(cp, ncp)) {
					marbles.add(convert(cp.red[0], cp.red[1]) + convert(cp.blue[0], cp.blue[1]));
					dq.offer(ncp);
				} else if (result == 1) {
					return ncp.depth;
				}
	...
	public static int moveMarble(Capture cp, int direction) {
		...
		if (marbles.contains(convert(ry, rx) + convert(by, bx))) {
			return -1;
		}
